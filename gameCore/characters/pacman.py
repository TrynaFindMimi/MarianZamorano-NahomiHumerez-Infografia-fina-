...
class Pacman(arcade.Sprite):
    ...
    def _try_direction(self, walls: arcade.SpriteList, vx: float, vy: float) -> bool:
        ...
        self.center_x += vx
        self.center_y += vy
        collided = arcade.check_for_collision_with_list(self, walls)
    ...
    def move(self, walls: arcade.SpriteList, tile_size: int = None):
        ...
        if self.dx != 0:
            ...
            if arcade.check_for_collision_with_list(self, walls):
                self.center_x = old_x
                self.dx = 0
                if self._try_direction(walls, 0, bounce_offset):
                    self.center_y += bounce_offset
                elif self._try_direction(walls, 0, -bounce_offset):
                    self.center_y -= bounce_offset
            elif self._aligned_axis(self.center_y):
                self.center_y = self._snap_axis(self.center_y)
        if self.dy != 0:
            old_y = self.center_y
            ...
            if arcade.check_for_collision_with_list(self, walls):
                self.center_y = old_y
                self.dy = 0
                if self._try_direction(walls, bounce_offset, 0):
                    self.center_x += bounce_offset
                elif self._try_direction(walls, -bounce_offset, 0):
                    self.center_x -= bounce_offset
            elif self._aligned_axis(self.center_x):
                self.center_x = self._snap_axis(self.center_x)
        puerta_y_min = 332
        puerta_y_max = 398
